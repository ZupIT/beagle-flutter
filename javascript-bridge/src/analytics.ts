...
export function analytics(): AnalyticsProvider {
    ...
    function getConfig() {
      ...
      const functionId = registerFunctionForSingleExecution(
        (argumentMap?: Record<string, any>) => {
          analyticsConfig = argumentMap as AnalyticsConfig
        }
      )
      sendMessage("analytics.getConfig", JSON.stringify({functionId}))
      return analyticsConfig
    }
  
    function createRecord(record: AnalyticsRecord) {
      sendMessage("analytics.createRecord", JSON.stringify(record))
    }
