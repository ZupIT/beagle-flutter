...
ElevatedButton createElevatedButton() {
  return ElevatedButton(
    key: submitKey,
    onPressed: () => {BeagleSubmitForm.submit(_mockContext, _mockElement)},
    child: Text(_labelSubmit),
  );
}

final elevatedButton = createElevatedButton();
final simpleForm = createBeagleSimpleForm(
    onValidationError: onValidationError, onSubmit: onSubmit);

BeagleSimpleForm createBeagleSimpleForm(
    {Function? onValidationError, required Function onSubmit}) {
...
class SimpleFormStateSpy extends BeagleSimpleFormState {
  SimpleFormStateSpy({required this.withInputErrors, required this.simpleForm});
...
void main() {
  setUpAll(() async {
    ...
    when(() => _mockContext.findAncestorWidgetOfExactType<BeagleSimpleForm>())
        .thenReturn(simpleForm);

    when(() => _mockContext.findAncestorStateOfType()).thenReturn(
        SimpleFormStateSpy(withInputErrors: false, simpleForm: simpleForm));
  });
  ...
  group('Given a BeagleSubmitForm', () {
    group('When I press the OK button without any validation error', () {
      testWidgets('Then it should call onSubmit callback',
        ...
        when(() => _mockContext.visitChildElements((element) {}))
            .thenAnswer((realInvocation) {});
    ...
    group(
        'When I press the OK button, and it has a validation error handled by me',
        () {
      testWidgets('Then it should call onSubmit callback',
          (WidgetTester tester) async {
        //Defines that validation error will not occurs
        when(() => _mockContext.findAncestorStateOfType()).thenReturn(
            SimpleFormStateSpy(withInputErrors: true, simpleForm: simpleForm));
        final expectedWarningMessage =
            'BeagleSimpleForm: has a validation error';
    ...
    group(
        'When I press the OK button, and it has a validation error not handled by me',
        () {
      testWidgets('Then it should call onSubmit callback',
          (WidgetTester tester) async {
        //Defines that validation error will not occurs
        final simpleFormSpy =
            createBeagleSimpleForm(onSubmit: onSubmit, onValidationError: null);
        when(() => _mockContext.findAncestorStateOfType()).thenReturn(
            SimpleFormStateSpy(
                withInputErrors: true, simpleForm: simpleFormSpy));
        when(() =>
                _mockContext.findAncestorWidgetOfExactType<BeagleSimpleForm>())
            .thenReturn(simpleFormSpy);
        final expectedFirstWarningMessage =
            'BeagleSimpleForm: has a validation error';
        final expectedLastWarningMessage =
            'BeagleSimpleForm: you did not provided a validation function onValidationError';

        didCallOnSubmit = false;
        didCallOnValidationError = false;
        await tester.pumpWidget(
            createWidget(onSubmit: onSubmit, onValidationError: null));
        await tester.pumpAndSettle();
